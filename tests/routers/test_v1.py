...
def mock_request():
    ...
    mock_req.state.token_info = {
        'user_id': 'test-user',
        'user_roles': {'api-home': ['create', 'read', 'read_own', 'list', 'list_own', 'update', 'update_own', 'delete', 'delete_own']}
    }
...
async def test_read_items(mock_check_permissions, mock_request):
    ...
    items = [
        HomeWrite(name="Item 1"),
        HomeWrite(name="Item 2")
    ]
...
async def test_update_existing_item(mock_check_permissions, mock_request):
    ...
    with patch('routers.v1.update_item') as mock_update:
        # Create a test item
        item = HomeWrite(name="Updated Item")
